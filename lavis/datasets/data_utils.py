...
def load_video_obj(vid_obj, n_frms=MAX_INT, height=-1, width=-1, sampling="uniform", mode='accurate'):
    container = VideoReader(vid_obj, height=height, width=width)

    vlen = len(container)
    start, end = 0, vlen

    # n_frms = min(n_frms, vlen) # some videos are too short (anet)

    if sampling == "uniform":
        frame_inds = np.arange(start, end, vlen / n_frms).astype(int)
    elif sampling == "headtail":
        indices_h = sorted(rnd.sample(range(vlen // 2), n_frms // 2))
        indices_t = sorted(rnd.sample(range(vlen // 2, vlen), n_frms // 2))
        frame_inds = indices_h + indices_t
    else:
        raise NotImplementedError

    if mode == 'accurate':
        frms = container.get_batch(frame_inds)
        # get_batch -> T, H, W, C
        ...
        for idx in frame_inds:
            ...
            frms.append(frame)
        frms = np.stack(frms, axis=0)
        frms = frms.transpose(3, 0, 1, 2).astype(np.float32)  # (C, T, H, W)
