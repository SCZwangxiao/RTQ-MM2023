...
from lavis.datasets.data_utils import load_video_obj
...
class BlipPostVideoBaseProcessor(BaseProcessor):
...
@registry.register_processor("blip_post_video_train")
class BlipPostVideoTrainProcessor(BlipPostVideoBaseProcessor):
    def __init__(
        self,
        image_size=384,
        mean=None,
        ...
        transform.extend(
            [
                ...
                self.normalize,
                CTHW2TCHW(),  # C, T, H, W -> T, C, H, W
            ]
        )

        self.transform = transforms.Compose(transform)

    def __call__(self, vid_obj):
        """
        Args:
            clip (torch.tensor): Video clip to be cropped. Size is (C, T, H, W)
        Returns:
    ...
    def from_config(cls, cfg=None):
        ...
        enable_randaug = cfg.get("enable_randaug", True)
        ...
        return cls(
            ...
            max_scale=max_scale,
            n_frms=n_frms,
            enable_randaug=enable_randaug,
        )


@registry.register_processor("blip_post_video_eval")
class BlipPostVideoEvalProcessor(BlipPostVideoBaseProcessor):
