...
def get_cluster_inter(width, block_id, args=None):
	...
	target_frames_blocks = [args.num_frames,] + list(args.target_frames_blocks)
	cluster_num_blocks = [args.num_patches,] + list(args.cluster_num_blocks)

	cluster_num = cluster_num_blocks[block_id + 1]
	before_cluster_num = cluster_num_blocks[block_id]
	after_block_frames = target_frames_blocks[block_id + 1]
	before_block_frames = target_frames_blocks[block_id]
	...
	return TokenClusterInter(algorithm=args.cluster_algo,
								...
								original_frame=args.num_frames,								
...
class TokenClusterInter(torch.nn.Module):
	def __init__(self, algorithm='kmediods++',
		...
		assert algorithm in ['kmediods++', 'pooling', 'sparse_sampling', 'spectral',
							 'temporal_shift', 'token_shift', 'random']
	...
	def forward(self, x):
		...
		if self.mean_residual:
			...
			residual_x = x.reshape(B, before_block_frames, num_tokens, width)
			# after_block_frames x [B, frame_duration, num_tokens, width]
			frame_split = [it.mean(dim=1) for it in torch.split(residual_x, frame_duration, dim=1)]
			# [B x target_frames, num_tokens, width]
			residual_x = torch.stack(frame_split, dim=1).contiguous().reshape(B * after_block_frames, num_tokens, width)
		else:
			residual_x = None
		
		# cluster
		if self.algorithm in ['kmediods++', 'spectral', 'random']:
			...
			if self.algorithm == 'kmediods++':
				...
				mediods_ids = torch.randint(0, res_tmp.shape[1], (res_tmp.shape[0], cluster_num))
				assign = None
			else:
